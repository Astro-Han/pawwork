...
- Confirm the V1 release PR targets `maint/v1` and all required CI checks are green.
- Confirm the version bump is merged into `maint/v1`.
- Confirm `maint/v1` is up to date locally:
...
```bash
gh workflow run build.yml --repo Astro-Han/pawwork --ref maint/v1 -f phase=submit -f channel=prod -f target=macos -f arch=arm64
gh workflow run build.yml --repo Astro-Han/pawwork --ref maint/v1 -f phase=submit -f channel=prod -f target=macos -f arch=x64
```
...
```bash
gh workflow run build.yml --repo Astro-Han/pawwork --ref maint/v1 -f phase=full -f channel=prod -f target=windows -f arch=x64
```

> **All three final targets must be the same build commit.** mac finalize rebuilds from the commit pinned by its submit snapshot tag (`source_sha`), while the Windows `full` build uses the current `maint/v1` HEAD (`github.sha`). If `maint/v1` moves between the macOS submits and the Windows build, the targets disagree and the auto-publisher (Step 4) refuses to publish — by design. Freeze `maint/v1` for the duration of the release, or confirm the Windows build's commit matches the macOS `source_sha` before expecting auto-publish to fire.
...
gh workflow run mirror-release-to-r2.yml --repo Astro-Han/pawwork --ref maint/v1 -f tag=vX.Y.Z
```
