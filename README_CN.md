...
| | 爪印 PawWork | Codex App | Claude Cowork |
|---|---|---|---|
| 开源 | 是（Apache-2.0） | 否 | 否 |
| 自带 API Key | 支持 75+ 提供商 | 仅 OpenAI | 仅 Anthropic |
| ChatGPT OAuth | 支持 | 不支持 | 不支持 |
| 免费额度 | 有（OpenCode Zen） | 有限 | 有限 |
| 桌面原生应用 | macOS + Windows | 浏览器 | 浏览器 |
| 本地文件访问 | 完整工作区 | 沙箱 | 沙箱 |
| 本地模型 | 支持（Ollama、LM Studio 等） | 不支持 | 不支持 |
