...
function sampleAccount(platform: RemotePlatform): RemoteAccount {
  switch (platform) {
    ...
    case "feishu":
      return { platform, appId: "cli_x", appSecret: "sec", domain: "feishu", allowChat: "oc_chat" }
    case "wechat":
      return { platform, botToken: "tok", baseURL: "https://r2", allowFrom: "u@im.wechat" }
  }
}
...
function fakePairer(platform: RemotePlatform, over: Partial<PlatformPairer> = {}): PlatformPairer {
  return {
    ...
    async pair(_start, emit) {
      emit({ phase: "awaitingBind", platform, hint: platform === "feishu" ? "group" : "message" })
      return sampleAccount(platform)
    },
    makePlatform: () => ({ name: platform, start: async () => {}, reply: async () => {}, send: async () => {}, stop: async () => {} }),
    audience: () => (platform === "feishu" ? { allow_chat: "oc_chat", group_only: true } : { allow_from: "42" }),
    identity: () => ({ id: platform === "feishu" ? "oc_chat" : "42", name: `${platform} target` }),
    ...over,
  }
}
...
function deps(over: Partial<RemoteBridgeDeps> = {}): RemoteBridgeDeps {
  return {
    ...
    pairers: [fakePairer("telegram"), fakePairer("feishu"), fakePairer("wechat")],
    ...over,
  }
}
...
test("startPairing on an unknown platform emits an error", async () => {
  const runtime = new RemoteBridgeRuntime(deps({ pairers: [fakePairer("telegram")] }))
  const events = recordPairing(runtime)
  await runtime.startPairing("feishu")
  expect(events).toEqual([{ phase: "error", platform: "feishu", message: "unsupported platform feishu" }])
})
...
test("two platforms connect concurrently; disconnecting one keeps the other connected", async () => {
  const store = memoryStore()
  const runtime = new RemoteBridgeRuntime(deps({ credentials: store }))
  await runtime.startPairing("telegram", { token: "x" })
  await runtime.confirmPairing("telegram")
  await runtime.startPairing("feishu")
  await runtime.confirmPairing("feishu")
  expect(runtime.getStatus().channels.map((c) => c.platform).sort()).toEqual(["feishu", "telegram"])
  expect(store.value.map((a) => a.platform).sort()).toEqual(["feishu", "telegram"])

  await runtime.disconnect("telegram")
  const channels = runtime.getStatus().channels
  expect(channels.map((c) => c.platform)).toEqual(["feishu"])
  expect(channels[0].state).toBe("connected")
  expect(store.value.map((a) => a.platform)).toEqual(["feishu"])
  await runtime.stop()
})

test("startIfConfigured connects saved accounts; with none it stays empty", async () => {
  const runtime = new RemoteBridgeRuntime(deps({ credentials: memoryStore([sampleAccount("wechat")]) }))
  await runtime.startIfConfigured()
  const channels = runtime.getStatus().channels
  expect(channels.map((c) => c.platform)).toEqual(["wechat"])
  expect(channels[0].state).toBe("connected")
  await runtime.stop()
