...
describe("PawWork DSH client product layer", () => {
  ...
  test("surfaces automations directly below New Session through the public sidebar action slot", () => {
    ...
    const railButton = registration!.component({ wide: false })
    expect(railButton.props.children[0].props.width).toBe(18)
    expect(railButton.props.children[0].props.height).toBe(18)
    ;(button.props.onClick as () => void)()
