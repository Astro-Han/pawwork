...
export function feedbackDialogLabels(locale: MenuLocale) {
  const labels = {
    en: {
      ...
      message:
        "PawWork will copy a problem report to your clipboard and open the feedback form. The report may include app diagnostics, recent app logs, current session messages, tool output, file names, paths that include your system username, and file snippets. Review it before submitting.",
    ...
    zh: {
      title: "复制问题报告？",
      message:
        "PawWork 会复制一份问题报告到剪贴板，并打开反馈表单。报告可能包含应用诊断信息、最近应用日志、当前会话消息、工具输出、文件名、包含系统用户名的路径和文件片段。提交前请先检查。",
      confirm: "复制报告并打开表单",
      cancel: "取消",
      failedTitle: "问题报告失败",
