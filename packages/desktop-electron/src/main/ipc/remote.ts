 ...
 * crosses once, inbound, inside the start-pairing options. confirm/disconnect carry
 * no secret, and the status read back is always masked. Status and pairing progress
 * are broadcast to every window so the page reflects connect/degraded and the
 * bind → captured steps without polling.
 */
