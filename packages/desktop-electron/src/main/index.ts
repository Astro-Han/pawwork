...
const reportProblem = createFeedbackHandler({
  ...
  confirm: async (context) => {
    const labels = feedbackDialogLabels((context as DesktopContext | undefined)?.locale ?? menuLocale)
  ...
  diagnostics: (context) => diagnostics(context as DesktopContext),
  logTail: tail,
  sessionExport: (context) => sessionExport(context as DesktopContext),
  onError: async (error) => {
