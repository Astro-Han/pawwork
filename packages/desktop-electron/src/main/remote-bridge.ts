...
import type { FeishuDomain } from "@opencode-ai/remote-bridge/platforms/feishu/registration"
 ...
 * account, so the runtime holds at most one telegram, one feishu, one wechat. The
 * secret (bot token / app secret) lives here and is never sent back over IPC.
 */
export type RemoteAccount =
  | { platform: "telegram"; token: string; allowFrom: string; userName?: string }
  | { platform: "feishu"; appId: string; appSecret: string; domain: FeishuDomain; allowChat: string; chatName?: string }
...
export type PairingProgress = (event: Extract<RemotePairingEvent, { phase: "qr" } | { phase: "awaitingBind" }>) => void

/**
 * One platform's connect logic, isolated so the runtime stays platform-agnostic.
 * `pair` runs the scan-to-connect flow (emitting QR / bind progress) and resolves
 * with the account to save, or null if cancelled/aborted; a thrown error becomes a
...
export interface RemoteBridgeDeps {
  ...
  locale: () => string
  // Injected so the lifecycle can be tested without Electron, the Lark SDK, or a
  // live network: a fake buildApp and fake pairers exercise the whole runtime.
 ...
 * own restart loop) and report status independently — a dead Feishu token shows
 * `degraded` while Telegram stays `connected`. Every lifecycle change (confirm /
