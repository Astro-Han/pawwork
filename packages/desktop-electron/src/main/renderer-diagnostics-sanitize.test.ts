...
describe("renderer diagnostics sanitizer", () => {
    ...
    const event = sanitizeRendererDiagnosticEvent(
      {
        ...
        data: {
          ...
          mode_after: "reading_history",
          intent_type: "wheel_scroll",
          intent_source: "scroll_view",
          observation_type: "scroll_sample",
          accepted: true,
          anchor_changed: true,
          reason: "user_upward_navigation",
          anchor_kind: "reading",
  ...
  test("accepts typed session timeline reconcile diagnostics", () => {
    const event = sanitizeRendererDiagnosticEvent(
      {
    ...
    expect(event).toMatchObject({
      "event.name": "session.timeline.reconcile",
      route_session_id: "ses_route",
      visible_session_id: "ses_visible",
      timeline_session_id: "ses_timeline",
      data: {
