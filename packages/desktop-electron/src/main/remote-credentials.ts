...
// Chat accounts (bot tokens, app secrets, paired ids) are secrets. A secret crosses
// IPC exactly once, inbound, during pairing — the Telegram token when pasted, the
// Feishu/WeChat credentials never (they are minted main-side from a QR scan). From
