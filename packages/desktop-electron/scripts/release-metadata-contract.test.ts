...
function writeLatest(dir: string, filename: string, url: string, options: { sha512?: string; size?: number } = {}) {
...
function writeFakeGh(binDir: string, downloads: Record<string, string | FixtureFile> = {}, downloadFailure?: string) {
  ...
  writeFileSync(
    ...
    [
      ...
      "  }",
      "  mkdirSync(dir, { recursive: true })",
      "  if (downloads[pattern]) writeFileSync(join(dir, pattern), latestYml(downloads[pattern]), 'utf8')",
      "  process.exit(0)",
      "}",
