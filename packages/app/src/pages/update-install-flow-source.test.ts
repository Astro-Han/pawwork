...
describe("update install renderer contracts", () => {
  test("renderer install actions do not relaunch after platform update", () => {
    ...
    expect(updatePolling).not.toMatch(/await\s+platform\.restart!\(\)/)
    ...
    expect(updatePolling).not.toMatch(/\.then\(\(\)\s*=>\s*platform\.restart!\(\)\)/)
  ...
  test("renderer update prompts only require platform update", () => {
    expect(updatePolling).toContain("if (!checkUpdate || !update) return")
    expect(updatePolling).not.toContain("restart")
