...
export const RemotePage: Component = () => {
  ...
  const openDisconnect = () => {
    // Inline, not a lazy module: it is a trivial confirm whose deps are already in
    // this page's bundle, so a separate dynamic chunk bought nothing. The heavy
    // connect/pairing flow stays lazily imported.
    dialog.show(() => <DialogDisconnectRemote />)
  }
...
// two-button Dialog with a danger action, per the design system. Inlined here
// (not a separate lazy module): it is a trivial confirm whose deps already ship
// with this page, so it never pulls in the connect flow's pairing state machine.
function DialogDisconnectRemote() {
  const language = useLanguage()
  const dialog = useDialog()
