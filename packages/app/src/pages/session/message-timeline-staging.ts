...
/**
 * Slice 11b.1: timeline staging logic extracted from `message-timeline.tsx`
 * per design doc §3b.
 *
 * `createTimelineStaging` defer-mounts small timeline windows so that
 * revealing older turns does not block first paint with a large DOM
 * mount. Once staging completes for a session it never re-stages —
 * backfill and new messages render immediately.
 */
