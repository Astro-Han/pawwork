...
// The order channels render in — Telegram (shipped first), then the Wave-1 adds.
const PLATFORMS: RemotePlatform[] = ["telegram", "feishu", "wechat"]
...
export function RemoteSurface(props: { onClose: () => void }) {
  ...
  const applyStatus = (status: RemoteStatus) => {
    ...
    setChannels({ telegram: byPlatform.telegram, feishu: byPlatform.feishu, wechat: byPlatform.wechat })
  }
  ...
  return (
    ...
    >
      <div class="mx-auto w-full max-w-[760px] px-6 py-6">
        ...
        <p class="mt-3 max-w-[680px] text-small text-fg-weak">{language.t("remote.wechat.note")}</p>
        <Show when={!supported}>
