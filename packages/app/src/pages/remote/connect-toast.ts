import type { RemoteState } from "@/desktop-api-contract"
 ...
 * auto-reconnect stays silent. It fires exactly when a channel reaches "connected"
 * — not the moment Allow returns, which is a step before the bridge is actually
 * serving. A terminal non-connected outcome ("degraded", e.g. a revoked token, or
 * "disconnected") disarms with no toast (the channel row already shows the cause);
 * "connecting" keeps waiting.
 */
export function connectToastAction(awaiting: boolean, next: RemoteState): ConnectToastAction {
