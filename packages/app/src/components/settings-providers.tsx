    ...
    <div class="flex flex-col h-full overflow-y-auto no-scrollbar px-4 pb-10 sm:px-10 sm:pb-10">
      <div class="sticky top-0 z-10 bg-[linear-gradient(to_bottom,var(--surface-raised)_calc(100%_-_24px),transparent)]">
        <div class="flex flex-col gap-1 pt-6 pb-8 max-w-[720px]">
          <h2 class="text-h2 text-fg-strong">{language.t("settings.providers.title")}</h2>
        </div>
      </div>

      <div class="flex flex-col gap-8 max-w-[720px]">
        <div class="flex flex-col gap-1" data-component="connected-providers-section">
          <h3 class="text-h3 text-fg-strong pb-2">{language.t("settings.providers.section.connected")}</h3>
          <SettingsList>
            <Show
                ...
                {(item) => (
                  ...
                  >
                    <div class="flex items-center gap-3 min-w-0">
                      ...
                      <span class="text-h3 text-fg-strong truncate">{item.name}</span>
                      <Tag>{type(item)}</Tag>
                    </div>
                    <Show
                      when={canDisconnect(item)}
                      fallback={
                        <span class="text-body text-fg-base opacity-0 group-hover:opacity-100 transition-opacity duration-200 pr-3 cursor-default">
        ...
        <div class="flex flex-col gap-1">
          <h3 class="text-h3 text-fg-strong pb-2">{language.t("settings.providers.section.popular")}</h3>
          <SettingsList>
            <For each={popular()}>
              {(item) => (
                <div
                ...
                >
                  <div class="flex flex-col min-w-0">
                    <div class="flex items-center gap-x-3">
                      <ProviderIcon id={item.id} class="size-5 shrink-0 text-icon-strong" />
                      <span class="text-h3 text-fg-strong">{item.name}</span>
          ...
          <Button
            variant="ghost"
            class="px-0 py-0 mt-5 text-h3 text-brand-primary text-left justify-start hover:bg-transparent active:bg-transparent"
            onClick={() => {
