...
/**
 * Slice 11b.1: paced-streaming text helpers + the `<MessageMarkdown>` /
 * `<PacedMarkdown>` wrappers that the assistant message body uses.
 *
 * These were inline at the top of `message-part.tsx`. Moving them out
 * lets the text / reasoning renderers (and tool renderers that show
 * paths) import the markdown stack without dragging the rest of the
 * message-part dispatcher's transitive imports.
 *
 * The `createPacedValue` hook keeps a 24-ms cadence "reveal" of a
 * streaming text part. Render-loop cadence: snap at whitespace /
 * punctuation if available within an 8-char lookahead so the visible
 * tail breaks cleanly. Reset to the source when streaming stops, when
 * the source string shrinks (truncate after retry), or when the source
 * stops being a prefix of the shown text (out-of-order edit).
 */
...
export function joinWorkspacePath(directory: string, relative: string): string {
