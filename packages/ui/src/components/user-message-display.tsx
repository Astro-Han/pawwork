...
import type { FilePart, TextPart, UserMessage, Part as PartType } from "@opencode-ai/sdk/v2"
 ...
 *
 * `UserMessageDisplay` renders the legacy user-side surface — the
 * pre-rewrite bubble layout used by the existing SessionTurn shell.
 * The W1 rewrite introduces a new cream bubble via
 * `session-turn-user-bubble.tsx`; both surfaces coexist by intent
 * through this slice (the public API stays the same so callers can
 * switch by import path).
 *
 * `HighlightedText` is a sibling helper that styles inline file
 * references inside the user-message body — co-located here because
 * UserMessageDisplay is the sole consumer.
 */
