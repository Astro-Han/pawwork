...
Only `python-pptx` (see `../../route-templates/svg-pptx/pyproject.toml`, pinned `python-pptx==1.0.2`, Python >= 3.10). `XlsxWriter`/`openpyxl`/`cairosvg`/`Pillow` are all optional and deliberately not included.
