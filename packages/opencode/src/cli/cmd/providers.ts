...
export const ProvidersListCommand = cmd({
  ...
  async handler(_args) {
    ...
    for (const [providerID, provider] of Object.entries(database as Record<string, { env: string[]; name?: string }>)) {
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (provider === "other") {
          ...
          prompts.log.warn(
            `This only stores a credential for ${provider} - you will need configure it in pawwork.json, check the docs for examples.`,
          )
        }
        ...
        if (provider === "amazon-bedrock") {
          prompts.log.info(
            "Amazon Bedrock authentication priority:\n" +
              ...
              "Configure via pawwork.json options (profile, region, endpoint) or\n" +
