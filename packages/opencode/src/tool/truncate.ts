...
export namespace Truncate {
  ...
  function hasAgentTool(agent?: Agent.Info) {
    if (!agent?.permission) return false
    return evaluate("agent", "*", agent.permission).action !== "deny"
  }
    ...
    Effect.gen(function* () {
      ...
      const output = Effect.fn("Truncate.output")(function* (text: string, options: Options = {}, agent?: Agent.Info) {
        ...
        const hint = hasAgentTool(agent)
          ? `The tool call succeeded but the output was truncated. Full output saved to: ${file}\nUse the agent tool (subagent dispatch) to have the explore subagent process this file with Grep and Read (with offset/limit). Do NOT read the full file yourself - delegate to save context.`
...
export const DIR = Truncate.DIR
export const GLOB = Truncate.GLOB
export type Result = Truncate.Result
