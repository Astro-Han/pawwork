...
export namespace SessionEntry {
  ...
  export function step(old: History, event: SessionEvent.Event): History {
    const draft = structuredClone(old) as Mutable<History>
    const lastAssistant = draft.entries.findLast(
      (x: Mutable<Entry>): x is Mutable<Assistant> => x.type === "assistant",
    )
    ...
    switch (event.type) {
      case "prompt": {
        if (pendingAssistant) {
          draft.pending.push(User.fromEvent(event) as Mutable<User>)
          break
        }
        draft.entries.push(User.fromEvent(event) as Mutable<User>)
        break
      }
      case "step.started": {
      ...
      case "text.delta": {
        if (!pendingAssistant) break
        const match = lastText(pendingAssistant)
      ...
      case "tool.called": {
        ...
        if (match) {
          ...
          match.state = {
            status: "running",
            input: event.input as Mutable<Record<string, unknown>>,
          }
        }
        break
      }
      case "tool.success": {
        ...
        if (match && match.state.status === "running") {
          match.state = {
            ...
            output: event.output ?? "",
            title: event.title,
            metadata: (event.metadata ?? {}) as Mutable<Record<string, unknown>>,
            attachments: (event.attachments ?? []) as Mutable<typeof event.attachments>,
          }
        }
        break
      }
