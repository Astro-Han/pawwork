...
- [x] `server/instance/config.ts` — migrated 2026-06-19. Get, update, and providers handlers now run through named route effects plus one shared AppRuntime bridge while keeping Hono validation, strict JSON handling, provider default model mapping, and OpenAPI metadata unchanged.
