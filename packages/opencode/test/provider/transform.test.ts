...
describe("ProviderTransform.message - surrogate sanitization", () => {
  ...
  test("replaces lone surrogates in model-visible text", () => {
    ...
    expect(result[4].content[2].output.value).toBe(expected("assistant tool text"))
    expect(result[4].content[3].output.value).toBe(expected("assistant tool error"))
    expect(result[4].content[4].output.value[0].text).toBe(expected("assistant tool content"))
...
describe("ProviderTransform.message - anthropic empty content filtering", () => {
  ...
  test("keeps empty Anthropic reasoning parts with signature metadata", () => {
    const msgs = [
      {
        role: "assistant",
        content: [
  ...
  test("keeps empty Bedrock reasoning parts with signature or redacted data metadata", () => {
    const bedrockModel = {
      ...anthropicModel,
