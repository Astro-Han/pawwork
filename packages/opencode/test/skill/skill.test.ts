...
test("discovers bundled skills from the repo skills directory in dev", async () => {
  ...
  try {
    await Instance.provide({
      ...
      fn: async () => {
        const skills = await runSkill((skill) => skill.all())
        // Positive: the office-* native skills must be present. Missing entries signal a broken
        // repo skills directory that would ship an incomplete bundle.
        const bundledNames = ["office-docx", "office-pdf", "office-pptx", "office-xlsx"]
        for (const name of bundledNames) {
          expect(skills.find((item) => item.name === name)).toBeDefined()
        }
        // Removal guard: the officecli/morph skill bundle was retired in the office-* switchover
        // (#1273). No skill may carry the retired prefixes, so a future sync cannot silently
        // re-vendor them.
        const retiredPrefix = /^(officecli-|morph-ppt)/
        expect(skills.filter((item) => retiredPrefix.test(item.name)).length).toBe(0)
