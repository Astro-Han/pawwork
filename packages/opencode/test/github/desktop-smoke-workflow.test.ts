...
describe("desktop smoke workflow", () => {
  test("defines a PR-safe macOS arm64 smoke build", () => {
    ...
    const prepareUvStep = smokeSteps.find((step) => step.name === "Prepare uv")
    ...
    expect(changesCheckoutStep?.uses).toBe("actions/checkout@9c091bb21b7c1c1d1991bb908d89e4e9dddfe3e0")
    ...
    expect(prepareUvStep).toBeDefined()
    expect(prepareUvStep?.run).toBe("bun ./scripts/prepare-uv.ts --platform darwin --arch arm64")
    expect(prepareUvStep?.["working-directory"]).toBe("packages/desktop-electron")
    ...
    expect(smokeSteps.indexOf(prepareUvStep!)).toBeGreaterThan(smokeSteps.indexOf(smokeBunStep!))
    expect(smokeSteps.indexOf(prepareUvStep!)).toBeLessThan(smokeSteps.indexOf(buildStep!))
