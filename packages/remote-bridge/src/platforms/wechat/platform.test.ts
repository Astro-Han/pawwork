...
import { captureWeChatSender, inboundMessage, WeChatPlatform, type WeChatTransport } from "./platform.ts"
...
test("inboundMessage with empty allowFrom accepts any sender (capture mode)", () => {
  expect(inboundMessage(wxMsg({ fromUserId: "anyone@im.wechat" }), "")?.userID).toBe("anyone@im.wechat")
})
...
test("start fires onReady, routes a paired message, and replies with its context token", async () => {
  ...
  let ready = false
  const run = platform.start((_p: Platform, msg) => received.push(msg.content), () => {
    ready = true
  })
...
test("captureWeChatSender returns the first inbound sender", async () => {
  const transport = new FakeTransport()
  transport.pushBatch([wxMsg({ messageType: 2 }), wxMsg({ fromUserId: "u_first@im.wechat" })])
  const controller = new AbortController()
  expect(await captureWeChatSender(transport, controller.signal)).toBe("u_first@im.wechat")
})

test("captureWeChatSender returns null when aborted before a sender arrives", async () => {
  const transport = new FakeTransport()
  const controller = new AbortController()
  const capture = captureWeChatSender(transport, controller.signal)
  controller.abort()
