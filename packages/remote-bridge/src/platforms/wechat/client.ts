...
// (the WeChat ClawBot slot, released 2026-03): a NAT-friendly HTTP long-poll —
...
export interface WeChatQrcode {
  /** Opaque handle to poll status with, and the value the QR encodes. */
  qrcode: string
  /** Pre-rendered QR image (data URL / base64) the UI can show directly. */
  qrcodeImg: string
}
...
/** A urlencoded random per-request UIN header iLink expects. */
function uin(): string {
...
export class WeChatClient {
  ...
  async getBotQrcode(signal?: AbortSignal): Promise<WeChatQrcode> {
    ...
    return { qrcode: str(data, "qrcode"), qrcodeImg: str(data, "qrcode_img_content") }
  }

  /** Poll whether the user has scanned + confirmed; on confirm, returns the
   * bot token and the base URL to use for all later calls. */
  async getQrcodeStatus(qrcode: string, signal?: AbortSignal): Promise<WeChatLoginStatus> {
    ...
    if (str(data, "status") === "confirmed" && botToken !== "") {
      return { status: "confirmed", botToken, baseURL: str(data, "baseurl") || this.baseURL }
    }
