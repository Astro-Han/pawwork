...
test("getBotQrcode returns the QR handle and image", async () => {
  const server = mockServer((_req, url) => {
    ...
    return json({ qrcode: "QR123", qrcode_img_content: "data:image/png;base64,AAA" })
  })
  try {
    const qr = await new WeChatClient({ baseURL: server.url }).getBotQrcode()
    expect(qr).toEqual({ qrcode: "QR123", qrcodeImg: "data:image/png;base64,AAA" })
  } finally {
    server.stop()
  }
})

test("getQrcodeStatus reports waiting, then confirmed with token + base url", async () => {
  let calls = 0
  const server = mockServer(() => {
    calls++
    return calls === 1
      ? json({ status: "waiting" })
      : json({ status: "confirmed", bot_token: "tok_abc", baseurl: "https://r2.ilinkai.weixin.qq.com" })
  })
  try {
